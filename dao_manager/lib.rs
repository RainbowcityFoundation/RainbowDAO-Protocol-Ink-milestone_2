...
mod dao_manager {
    ...
    impl DAOManager {
        ...
        #[ink(message)]
        pub fn get_dao_category(&self) -> String {
            self.category.clone()
        }
        ...
        fn _init_base(&mut self, base_code_hash: Hash,
                     version: u128,logo:String,name:String,desc:String) -> bool {
            let salt = version.to_le_bytes();
            let total_balance = Self::env().balance();
            assert!(total_balance > CONTRACT_INIT_BALANCE, "not enough unit to instance contract");
            // instance base
            ...
            contract_instance.init_base(name, logo, desc);
        ...
        fn _init_erc20(&mut self, erc20_code_hash: Hash,
                      version: u128,total_supply:u128,name:String,symbol:String,decimals:u8,owner:AccountId) -> bool {
            let salt = version.to_le_bytes();

            let total_balance = Self::env().balance();
            assert!(total_balance > CONTRACT_INIT_BALANCE, "not enough unit to instance contract");
            // let vault_addr = self.component_addrs.vault_addr.unwrap();
            let erc20_instance_params = Erc20::new(total_supply,name, symbol, decimals, owner)
                .endowment(CONTRACT_INIT_BALANCE)
